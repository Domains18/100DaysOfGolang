...
func getIDFromRequest(formValue string, ctx *gin.Context)(int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to parse ID form value %s: %w", formValue, err)
	}
	return id, nil
}
func SendKafkaMessage(producer sarama.SyncProducer, users []models.User, ctx *gin.Context, fromID, toID int) error {
	...
	notificationToJSON, err := json.Marshal(notification)
	if err != nil {
	...
	msg := &sarama.ProducerMessage{
		...
		Value: sarama.StringEncoder(notificationToJSON),
	}
